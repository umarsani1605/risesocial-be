...
describe('Admin Placement Endpoints (RS-28)', () => {
  ...
  describe('POST /admin/cohort-placements/:id/transfer', () => {
    it('transfers placement to new cohort', async () => {
      ...
      const placement = await prisma.cohortPlacement.create({
        data: { academy_enrollment_id: enrollment.id, cohort_id: cohortA.id, user_id: user.id, academy_id: academy.id },
      });
      ...
      const res = await app.inject({
        ...
        headers: { authorization: `Bearer ${adminToken}`, 'content-type': 'application/json' },
        payload: { cohort_id: cohortB.id },
      });

      expect(res.statusCode).toBe(200);
      const body = res.json();
      expect(body.data.cohort_id).toBe(cohortB.id);

      // Old placement should be gone
